...
/** JMH validation benchmarks to test JVM optimization behavior.
  *
  * These benchmarks help validate that the JVM's optimizations (Dead Code Elimination,
  * Constant Folding, etc.) are working as expected and that our main benchmarks are
  * actually measuring real work rather than being optimized away.
  *
...
class ValidationBench {
  ...
  /** Test for Dead Code Elimination - this should be optimized to near-zero time.
    *
    * This benchmark creates Eru effects and compositions but doesn't consume
    * their results. If Dead Code Elimination is working properly, the JVM
    * should optimize away most or all of this work, resulting in very fast
    * execution times.
    *
    * If this benchmark shows significant execution time, it indicates that
    * either DCE isn't working or our effects have unavoidable side effects.
    */
  @Benchmark
  ...
  /** Test for Dead Code Elimination with controlled consumption.
    *
    * This is the control for the deadCodeEliminationTest. It does the same
    * work but consumes the result through blackhole. The difference between
    * this and deadCodeEliminationTest shows how much work the JVM eliminated.
    */
  @Benchmark
  ...
  /** Test for Constant Folding - this should be very fast due to compile-time optimization.
    *
    * This benchmark performs operations that should be foldable to constants
    * at compile time. If constant folding is working, this should execute
    * much faster than equivalent dynamic computation.
    */
  @Benchmark
  ...
  /** Test for Constant Folding control with dynamic values.
    *
    * This benchmark performs similar operations to constantFoldingTest but
    * with dynamic values that can't be folded at compile time. The difference
    * shows how much constant folding helped in the previous test.
    */
  @Benchmark
  ...
  /** Test blackhole effectiveness - without blackhole consumption.
    *
    * This benchmark performs meaningful work but doesn't consume the results
    * through blackhole. If blackholes are necessary, this should be much
    * faster than the blackhole version due to dead code elimination.
    */
  @Benchmark
  ...
  /** Test blackhole effectiveness - with blackhole consumption.
    *
    * This benchmark performs the same work as blackholeValidationWithoutConsumption
    * but consumes the result through blackhole. The difference shows whether
    * blackhole consumption is necessary to prevent optimization.
    */
  @Benchmark
  ...
  /** Test for loop unrolling and optimization behavior.
    *
    * This benchmark tests how the JVM handles repetitive operations.
    * It helps validate that our main benchmarks with repeated operations
    * are measuring realistic performance rather than over-optimized scenarios.
    */
  @Benchmark
  ...
  /** Test allocation vs computation balance.
    *
    * This benchmark helps understand whether performance differences in
    * main benchmarks are due to computation overhead or allocation patterns.
    */
  @Benchmark
