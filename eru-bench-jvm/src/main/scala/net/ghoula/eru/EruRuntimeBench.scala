...
class EruRuntimeBench {
  ...
  def zipSequential(): Int = {
    ...
    val program = doWork(1).zip(doWork(2)).zip(doWork(3).zip(doWork(4)))
      .map { case ((r1, r2), (r3, r4)) => r1 + r2 + r3 + r4 }
  ...
  def ensureHandling(): String = {
    ...
    val program = Eru.succeed("main-result")
