...
/** Critical tests to verify structured concurrency guarantees.
  ...
  *
  * Coordination is purely spin-waiting on AtomicBoolean flags, with no timing assumptions, so the
  * assertions are deterministic: a child fiber either reaches its completion marker or is
  * interrupted by structured concurrency.
  */
