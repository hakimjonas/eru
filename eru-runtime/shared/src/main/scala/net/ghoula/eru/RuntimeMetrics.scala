...
/** Runtime metrics for transparent observability.
  *
  * Provides detailed insight into runtime behavior without impacting performance. All metrics are
...
/** Global metrics instance (optional - can create per-runtime). */
object RuntimeMetrics {
  val global: RuntimeMetrics = new RuntimeMetrics()
}
