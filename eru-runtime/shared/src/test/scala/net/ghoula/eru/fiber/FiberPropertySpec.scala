...
/** Property-based tests for fiber operations.
  *
  * Tests that fork/await operations are referentially transparent and preserve
  * fundamental properties like monad laws in the unified fiber runtime.
  */
class FiberPropertySpec extends FunSuite {
  ...
  test("fork/await preserves computation structure for complex effects") {
    ...
    val fetchOrders: Int => Eru[String, List[Order]] = userId =>
      Eru.succeed(List(Order(userId, 100.0), Order(userId, 200.0)))
