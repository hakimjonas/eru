...
/** Test to reproduce the specific resource leak pattern identified in FiberStressSpec.
  *
  * This test isolates the exact race condition where resource counters and cleanup counters
  * become misaligned under high concurrency pressure.
  */
class ResourceLeakReproductionSpec extends FunSuite {
  ...
  test("correct resource pattern - acquire and cleanup symmetrically") {
    ...
    assertEquals(cleanupCounter.get(), resourceCounter.get(), 
      "Correct pattern should never have resource leaks")
  }
}