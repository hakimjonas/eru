...
object EruRuntime {
  ...
  /** Forks an effect and returns a completed fiber computed synchronously.
    * This preserves the Fiber API without requiring a scheduler.
    */
  ...
  def sleep(duration: Duration): Eru[Nothing, Unit] =
    Eru.blocking {
      ...
      try Thread.sleep(ms) catch { case _: InterruptedException => () }
      ()
