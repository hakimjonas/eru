...
/** A per-key concurrency limiter backed by independent semaphores.
  ...
  * access. Permits are tracked per key, so `withPermit("a")(slow)` does not block
  * `withPermit("b")(fast)`.
  *
...
trait KeyedSemaphore[K] {
  ...
  /** Acquires a permit for `key`, runs `fa`, and releases it afterward.
    *
    * Suspends until a permit is available for the given key. The result is a `Suspending` effect,
    * matching `Semaphore.withPermit`: it cannot be run with `unsafeRunSync` without an explicit
    * `timeout` or `fork`.
    */
...
object KeyedSemaphore {
  ...
  private final class RuntimeKeyedSemaphore[K](
    ...
    def withPermit[E, A](key: K)(fa: => Eru[E, A]): Suspending[E, A] =
      Suspending(Eru.effectTotal(semaphoreFor(key)).flatMap(_.withPermit(fa).eru))
