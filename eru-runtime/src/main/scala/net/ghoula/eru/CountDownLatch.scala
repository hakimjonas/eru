...
trait CountDownLatch {
  ...
  /** Returns the current count value.
    ...
    */
  def count: Immediate[Nothing, Int]
  ...
  /** Checks whether the count has reached zero.
    ...
    */
  def isZero: Immediate[Nothing, Boolean] = new Immediate(count.eru.map(_ == 0))
}
...
object CountDownLatch {
  ...
  private final class RuntimeCountDownLatch(stateRef: Ref[LatchState], runtime: EruRuntime) extends CountDownLatch {
    ...
    def count: Immediate[Nothing, Int] = new Immediate(stateRef.get.map(_.count))
