...
trait RefMap[K, V] {
  ...
  /** Atomically updates the value for a key using `f`. No-op if the key doesn't exist.
    *
    * Uses `ConcurrentHashMap.computeIfPresent` for per-key atomicity — updates to different keys do
    * not contend beyond the map's internal bin structure.
    *
    * @return
  ...
  /** Returns an iterable view of the values.
    *
...
object RefMap {
  ...
  private final class RuntimeRefMap[K, V] extends RefMap[K, V] {
    ...
    def updateOrCreate(key: K, create: => V)(f: V => V): Eru[Nothing, V] =
      Eru.effectTotal {
        chm.compute(key, (_, existing) => Option(existing).fold(create)(f))
      }
