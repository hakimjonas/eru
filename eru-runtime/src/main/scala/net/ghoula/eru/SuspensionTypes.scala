...
/** A computation that may suspend indefinitely.
  ...
  * via fork, race, or timeout, which bound the suspension.
  *
  * This is a value class; the wrapper is erased where the compiler can avoid boxing, and may box in
  * generic contexts.
  *
  * @example
  *   {{{
...
final class Suspending[+E, +A](val eru: Eru[E, A]) extends AnyVal {
  ...
  /** Chains this suspending computation with another effect-returning function.
    ...
    *
    * val transfer: Suspending[Nothing, Unit] = queue1.take.flatMap(item => queue2.put(item).eru)
    *   }}}
    */
  ...
  def timeout(
    duration: Duration
  )(using runtime: EruRuntime, m: net.ghoula.eru.time.Monotonic): Immediate[E | Throwable, A] = {
    val _ = m
    val timeoutEru =
      runtime
        .sleep(duration)
        .flatMap(_ => Eru.fail(new java.util.concurrent.TimeoutException(s"Operation timed out after $duration")))
...
final class Immediate[+E, +A](val eru: Eru[E, A]) extends AnyVal {
  ...
  /** Safely run this non-suspending computation synchronously.
    ...
    * @throws Throwable
    *   if the computation fails with a throwable, which is rethrown as-is
    */
