...
trait Semaphore {
  ...
  /** Acquires n permits, one at a time.
    *
    * Each permit is acquired individually: the caller may hold some permits while suspending for
    * the rest. For n <= 0, this completes immediately without acquiring anything.
    *
    * @param n
    *   number of permits to acquire
    * @return
    *   a suspending effect that completes when n permits have been acquired
    */
...
object Semaphore {
  ...
  /** Creates a new semaphore initialized with `n` permits; negative values are clamped to 0. */
