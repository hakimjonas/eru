...
/** Virtual Thread implementation of AsyncScheduler.
  *
  * This scheduler provides true asynchronous execution using Java Virtual Threads,
  * enabling proper FILO finalizer semantics by allowing parent fibers to continue
  * executing while children run concurrently.
  */
...
/** Virtual Thread implementation of AsyncFiber.
  *
  * This fiber runs the computation on a Virtual Thread and provides callback
  * registration for async completion notification.
  */
  ...
  def await: Eru[Nothing, Exit[E, A]] = {
    ...
    Eru.blocking {
      ...
      case Result.Failure(_) => 
        ...
        val interruptFiber = EruFiber.withId(id, Exit.Interrupt(id, InterruptCause.Cancelled(Some("Await interrupted"))), Nil)
