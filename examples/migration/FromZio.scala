/** Migration Guide: From ZIO to Eru
  *
  * This example demonstrates how to migrate common ZIO patterns to Eru,
...
object FromZio extends App {
  ...
  def transformationsExample(): Unit = {
  ...
  def collectionsExample(): Unit = {
    ...
    val parallel = EruRuntime.parTraverse(items)(processItem)
    val parallelN = EruRuntime.foreachParN(2, items)(processItem)
  ...
  def iterationExample(): Unit = {
    ...
    var counter = 0
    def condition: Eru[Nothing, Boolean] = Eru.effect(counter < 3).mapError(_ => ())
    def body: Eru[Nothing, Unit] = Eru.effect { counter += 1; println(s"Body executed: $counter") }.mapError(_ => ())

    def whileLoop: Eru[Nothing, Unit] = {
  ...
  def migrationSummary(): Unit = {
    ...
    |   Eru: .fork, .zipPar, .race, EruRuntime.* methods
    |
    |4. Resources:
    ...
    |   Eru: Eru.traverse, EruRuntime.parTraverse
    |
    |6. Execution:
