...
```scala
// build.sbt
// The prelude (net.ghoula.eru.prelude) lives in eru-runtime.
libraryDependencies += "net.ghoula" %% "eru-core" % "@VERSION@"
libraryDependencies += "net.ghoula" %% "eru-runtime" % "@VERSION@"
```
...
- `Nothing`: the program cannot fail
- `String`: on success, the program produces a String
...
The `println` inside `Eru.effect` happens only when the program runs.
...
The `.attempt.map(_.fold())` pattern mirrors `either.fold(handleError, handleSuccess)`.
...
The `fallback` method recovers specific error cases with fallback values.
...
In Chapter 3, we'll examine the `Eru` type in detail, including the optimizations that keep composition cheap.