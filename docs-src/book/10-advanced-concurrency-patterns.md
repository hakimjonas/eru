...
Building on Chapter 9, this chapter covers parallel processing strategies, coordination primitives, resource-bounded concurrency, and racing patterns.

## Parallel collection processing

Processing collections in parallel while keeping safety and composability is a common need:
...
// parTraverse processes items in parallel, one fiber per element
...
```scala mdoc
// Create timeout patterns for service calls
def serviceWithTimeout[A](
  operation: Eru[String, A],
  timeoutMs: Long,
  fallback: A
): Eru[String | Throwable, A] = {
  import java.time.Duration

  // timeoutTo runs the fallback internally when the deadline passes,
  // so the timeout never appears in the error channel
...
def timeoutDemo(): Eru[String | Throwable, String] = {
  for {
...
case class CircuitBreakerState(
  failureCount: Int = 0,
  lastFailureTime: Long = 0L, // System.nanoTime(): monotonic, immune to wall-clock jumps
  isOpen: Boolean = false
)

// Functional circuit breaker using Ref for thread-safe state management.
// Note the monotonic clock: wall time (currentTimeMillis) can jump on NTP corrections and
// would shorten or lengthen the recovery window. Eru ships
// patterns.ErrorHandling.CircuitBreaker, which uses the same monotonic measurement.
class FunctionalCircuitBreaker(failureThreshold: Int, recoveryTimeoutNanos: Long) {
  ...
  def execute[A](operation: Eru[String, A]): Eru[String | Throwable, A] = {
    for {
      ...
      result <- if (state.isOpen && (currentTime - state.lastFailureTime) < recoveryTimeoutNanos) {
...
def circuitBreakerDemo(): Eru[String | Throwable, List[String]] = {
  val circuitBreaker = new FunctionalCircuitBreaker(failureThreshold = 2, recoveryTimeoutNanos = 1_000_000_000L)
...
When managing many independent resources (domains in a crawler, tenants in a SaaS system, connections per host), global primitives like `Ref[Map[K, V]]` or a single `Semaphore` require one lock or CAS for every operation. Eru provides two per-key primitives that remove that contention.

### RefMap: per-key atomic state

A `Ref[Map[K, V]]` requires a global CAS on every update, so updates to unrelated keys contend with each other. `RefMap` uses a `ConcurrentHashMap` internally, so updates to different keys do not share a lock:
...
def fetchPage(host: String, path: String): Eru[String | Throwable, String] = {
  ...
  }.eru // Suspending -> Eru: withPermit may block, so it suspends
}
...
The building blocks of concurrent systems:

Parallel processing: `parTraverse` forks one fiber per element; `foreachParN` bounds concurrency.

Coordination primitives: `Promise`, `Deferred`, and `Semaphore` coordinate independent fibers.

Resource bounding: semaphores and connection pools bound resource use and provide backpressure.

Racing: timeout patterns, circuit breakers, and fallback strategies handle failures and slow dependencies.

Pipeline processing: multi-stage pipelines compose from forked stages.

Error isolation: bulkhead patterns stop failures in one component from cascading to others.

Per-key primitives: `RefMap` and `KeyedSemaphore` keep updates to different keys lock-free relative to each other, so per-domain state tracking and per-host concurrency limits need no global bottleneck.

These patterns bound concurrency and isolate failures.

## What's next

Chapter 11 covers Eru's observability system: `EruObserver`, structured events, performance monitoring, and debugging techniques for concurrent programs.