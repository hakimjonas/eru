# Chapter 14: Libraries around Eru

This chapter places Eru among the other Scala effect systems, covers migration from ZIO and Cats Effect, and lists the libraries it pairs with.

## Eru's position

Scala has several effect systems, each with its own tradeoffs. Eru's position:

Correctness-first design: correctness comes first.

Direct API: combinators sit on the `Eru` type; a single import brings the prelude into scope.

Virtual thread foundation: designed for the JVM's virtual threads.

### Complementary tools

Eru works alongside other tools:
...
Practical strategies for migrating from Cats Effect and ZIO to Eru:
...
object CatsEffectMigration {
  ...
  // Eru equivalent: zipPar (concurrent), fork + await

  def concurrencyMigration(): Eru[String | Throwable, (String, String)] = {
    val task1 = Eru.succeed("Task 1")
    val task2 = Eru.succeed("Task 2")

    // Cats Effect: IO.both(task1, task2)
    // Eru equivalent — zip is SEQUENTIAL (left first, then right); zipPar runs both sides:
    task1.zipPar(task2)
  ...
  // Eru equivalent: attempt, recoverWith, fallback
...
object ZIOMigration {
  ...
  // ZIO Schedule -> Eru's retry with EruRuntime.Policy; shown
  // by hand here to make the shape visible
  def retryPattern[A](
...
Eru ships `retry` with `EruRuntime.Policy.NoDelay(n)` and `EruRuntime.Policy.Exponential(...)` (Chapter 5); the hand-rolled version above is for custom schedules.

## Patterns used in applications

Patterns used in Eru applications:
...
object FutureEvolution {
  ...
  def futureFeatureExample(): Eru[String, String] = {
    ...
    for {
      ...
      result <- Eru.succeed("Future Eru will have more abstractions")
    } yield result
  }
}
...
Greenfield projects: Eru fits new projects where patterns can be established from the start.

Performance-critical applications: when effect-processing performance matters.

Team learning: Eru's small API surface leaves fewer concepts to learn.
...
Complementary design: Eru works alongside existing Scala tools.

Migration support: the Cats Effect and ZIO sections give the migration paths.

Patterns: repositories, sagas, and event sourcing assemble from Eru's primitives.

Testing: the testkit ships `EruTestSuite` (a munit base), `EruTest` assertions, and the deterministic `TestClock`/`LogicalTestClock` — it plugs into your test framework rather than shipping one.

Future evolution: planned work covers performance and observability.

Adoption: greenfield, legacy, and hybrid adoption all work; the migration section has the plans.

## Conclusion

Eru is a typed effect system for Scala. The libraries and migration patterns in this chapter cover adopting it in new and existing projects.

## Acknowledgments

Eru builds on ideas from Cats Effect, ZIO, and Monix, and uses many of the patterns they established. The Scala community's work on typed functional programming made Eru possible. Thank you to everyone who contributed.
