# Chapter 4: Sequencing and composition

After learning to create individual Eru programs, the next step is combining them into larger computations. This chapter covers Eru's composition operators and when to use each one.
...
> ```
> This object is the single entry point to the Eru API.
...
Key insight: `flatMap` lets errors from any step propagate through the chain.
...
### zip: combining two computations

Use `zip` when you want to combine the results of two computations. `zip` evaluates the left side first, then the right:
...
Key insight: `zip` fails if either side fails. For genuinely parallel evaluation, use `zipPar` (Chapter 10); `zip` runs the two sides sequentially.
...
def buildChain(n: Int): Eru[Nothing, Int] = {
  // The interpreter keeps continuation chains right-associated for stack safety
...
The two sides run sequentially, so `zip` suits cheap computations. For parallel evaluation, use `zipPar` (Chapter 10).
...
Errors flow through composition in a specific way:
...
Transparent runtime: the structure of your composition reflects execution order.

## What's next

Chapter 5 covers error handling: the `E` in `Eru[E, A]`. Recovery from failures, error accumulation, and applications that handle error conditions gracefully.