...
This chapter covers integrating Eru with existing codebases: blocking operations, legacy systems, and third-party libraries.

## Integration philosophy

Eru's integration approach follows these principles:

- Gradual adoption: migrate incrementally, without big-bang rewrites
- Safety preservation: Eru's guarantees hold even when integrating unsafe code
- Performance awareness: keep overhead low while providing safety and observability
- Library compatibility: work with existing Scala and Java libraries

## Wrapping legacy code

A common integration scenario is wrapping existing synchronous code in Eru effects:
...
//
// Eru ships this: net.ghoula.eru.FutureInterop.fromFuture (blocking via Eru.blocking)
// and .toFuture (evaluates eagerly into a completed Future). For truly non-blocking
// interop, suspend on the callback instead — runtime.suspend registers a callback with
// the async source and completes without holding a thread.
...
object MigrationStrategies {
  ...
  def migrateOrdersBatch(orderIds: List[String]): Eru[String, List[Either[String, String]]] = {
    ...
    // Process orders, collecting both successes and failures
...
Gradual migration: wrap legacy code first, then introduce Eru-native implementations alongside it.

Safety preservation: `Eru.effect` and typed error handling keep safety guarantees intact when integrating unsafe operations.

Resource management: apply `bracket` and `ensure` consistently for cleanup in integration scenarios.

Performance: use connection pooling, caching, and batching at integration points.

Testing: mock implementations and Eru's deterministic test kit make integration code testable.

Library compatibility: integrate with Future, Try, JSON libraries, and configuration systems.

Monitoring: apply monitoring and error-handling patterns to integration code.

## What's next

Chapter 14 describes Eru's libraries, migration paths from other effect systems, and its roadmap.