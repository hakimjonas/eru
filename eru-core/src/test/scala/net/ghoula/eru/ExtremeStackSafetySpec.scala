...
/** Extreme stack safety tests for optimized collection methods.
  ...
  *
  * The effectful recursive test exercises the TailCalls interpreter path via `Eru.effect`, unlike
  * `Eru.succeed`, which fuses eagerly and bypasses the interpreter entirely. Without the
  * `Step(f, End())` optimization, `TailRec.Cont` nodes accumulate and exhaust memory.
  */
class ExtremeStackSafetySpec extends munit.FunSuite {
  ...
  test("foreach handles extremely large collections without stack overflow") {
    ...
    assertEquals(result.head, 2)
    assertEquals(result.last, extremeSize * 2)
  }
  ...
  test("collectAll handles extremely large effect collections without stack overflow") {
    ...
    assertEquals(result.size, extremeSize)
    assertEquals(result.head, 3)
    assertEquals(result.last, extremeSize * 3)
  }
  ...
  test("traverse handles extremely large collections without stack overflow") {
    ...
    assertEquals(result.size, extremeSize)
    assertEquals(result.head, 4)
    assertEquals(result.last, extremeSize * 4)
  }
  ...
  test("sequence handles extremely large effect collections without stack overflow") {
    ...
    assertEquals(result.size, extremeSize)
    assertEquals(result.head, 5)
    assertEquals(result.last, extremeSize * 5)
  }
  ...
  test("nested collection operations maintain stack safety") {
    ...
    assertEquals(result.head.head, 1001)
    assertEquals(result.last.last, outerSize * 1000 + innerSize)
  }
