...
/** Test suite for common error handling patterns in the Eru effect system.
  ...
  *
  * `RetryContext.elapsedTime` is measured on the monotonic clock from `startNanos`; time-based
  * tests inject a past origin to keep them deterministic without waiting on wall time.
  */
class ErrorHandlingSpec extends munit.FunSuite {
  ...
  test("RetryPolicy.timeLimited respects time limits") {

    val timeLimit = Duration.ofSeconds(1)
    val policy = RetryPolicy.timeLimited(timeLimit, Duration.ofMillis(100))
    val now = Instant.now()

    val recentContext = RetryContext(now, 10, Nil, System.nanoTime() - 500_000_000L)
    val oldContext = RetryContext(now, 10, Nil, System.nanoTime() - 2_000_000_000L)
  ...
  test("RetryContext tracks errors and elapsed time") {
    ...
    val context = RetryContext(Instant.now(), 3, Nil, System.nanoTime() - 5_000_000_000L)
      .withError("error1")
      .withError("error2")
      .withError("error3")
    ...
    assert(elapsed.toMillis >= 4500L, s"elapsed ${elapsed.toMillis}ms should be at least 4500ms")
  }
