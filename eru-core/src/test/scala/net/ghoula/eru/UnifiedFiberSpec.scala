...
/** Test suite for UnifiedFiber implementation and state transitions.
  ...
  * execution models. Concurrent `complete` calls on the same active fiber are not thread-safe; the
...
class UnifiedFiberSpec extends munit.FunSuite {
  ...
  test("UnifiedFiber.active creates fiber with Active state") {
    ...
    assertEquals(fiber.id, id)
    fiber.currentState match {
      case UnifiedFiberState.Active(latch, exitRef, threadRef, _, _, _) =>
  ...
  test("UnifiedFiber.setThread updates thread reference for active fiber") {
    ...
    fiber.currentState match {
      case UnifiedFiberState.Active(_, _, threadRef, _, _, _) =>
  ...
  test("UnifiedFiberState.Active stores coordination primitives correctly") {
    ...
    val state = UnifiedFiberState.Active(
      latch,
      exitRef,
      threadRef,
      observerRef,
      id,
      new AtomicReference[Option[InterruptCause]](None)
    )

    state match {
      case UnifiedFiberState.Active(l, e, t, o, fid, _) =>
