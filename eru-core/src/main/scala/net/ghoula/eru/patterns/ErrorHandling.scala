...
object ErrorHandling {
  ...
  /** Retry policies with configurable strategies.
    *
    * RetryPolicy is the set of retry strategies that can be applied to effects.
    *
    * All policies support:
    *   - Retry decisions based on the error and the attempt count (`TimeLimited` uses elapsed time
    *     instead)
    ...
    * val policy =
    *   RetryPolicy.conditional[NetworkError](
    *     shouldRetryError = _.isTransient,
    *     maxAttempts = 3,
    *     baseDelay = Duration.ofMillis(100)
    *   )
    *
    * // Jittered exponential backoff
    * val jitteredPolicy = RetryPolicy.jitteredExponential(
    *   maxAttempts = 5,
  ...
  enum RetryPolicy {
    ...
    /** Conditional retry with exponential backoff based on error type predicate.
      ...
      * @param maxAttempts
      *   maximum number of total attempts (initial attempt included) before giving up
      * @param baseDelay
  ...
  /** Context information available during retry decisions.
    *
    * `startTime` (wall) is kept for display and correlation. `elapsedTime` is measured on the
    * monotonic clock from `startNanos` (default: the time of construction), so wall-clock
    * adjustments neither shorten nor lengthen the retry window. Passing an explicit `startNanos`
    * also gives tests deterministic control over the elapsed time without a clock abstraction.
    */
  final class RetryContext(
    ...
    /** Total elapsed time since the first attempt, measured on the monotonic clock. */
    def elapsedTime: Duration = Duration.ofNanos(System.nanoTime() - startNanos)

    /** Adds an error to the context history.
      *
      * The history is capped at the 10 most recent errors; older entries are dropped.
      */
    def withError(error: Any): RetryContext =
      RetryContext(startTime, totalAttempts, (error :: lastErrors).take(10), startNanos)
  ...
  object RetryContext {
    ...
    ): RetryContext = new RetryContext(startTime, totalAttempts, lastErrors, startNanos)
  }
  ...
  object RetryPolicy {
    ...
    /** Retry based on error type predicate with exponential backoff.
      *
      * The predicate is invoked only for values of type `E`; non-`E` values are treated as not
      * retryable.
      */
    def conditional[E](
    ...
    def timeLimited(
      ...
      RetryPolicy.TimeLimited(timeLimit, baseDelay)
    }
  ...
  private final class CircuitBreakerState(
    ...
    val failures: Long,
    val successes: Long,
    val lastFailureNanos: Option[Long]
  ) {
    ...
    def copy(
      ...
      lastFailureNanos: Option[Long] = this.lastFailureNanos
    ): CircuitBreakerState =
      CircuitBreakerState(state, failures, successes, lastFailureNanos)
    ...
    override def equals(obj: Any): Boolean = obj match {
      case that: CircuitBreakerState =>
        ...
        lastFailureNanos == that.lastFailureNanos
      case _ => false
    }

    /** Hash code based on all fields. */
    override def hashCode(): Int =
      DataClassUtils.hashCodeFor(state, failures, successes, lastFailureNanos)

    /** String representation for debugging. */
    override def toString: String =
      DataClassUtils.toStringFor("CircuitBreakerState", state, failures, successes, lastFailureNanos)
  }
  ...
  private object CircuitBreakerState {
    ...
    def apply(
      ...
      failures: Long,
      successes: Long,
      lastFailureNanos: Option[Long]
    ): CircuitBreakerState =
      new CircuitBreakerState(state, failures, successes, lastFailureNanos)
  }
  ...
  final class CircuitBreaker(
    ...
    private def shouldAttemptRecovery(state: CircuitBreakerState): Boolean = {
      state.lastFailureNanos match {
        case Some(lastFailureNanos) =>
          Duration.ofNanos(System.nanoTime() - lastFailureNanos).compareTo(recoveryTimeout) >= 0
        case None => true
      }
    }
  ...
  final class ErrorAccumulator[E](val errors: List[ErrorWithContext[E]]) {
    ...
    def firstError: Option[E] = errors.lastOption.map(_.error)
