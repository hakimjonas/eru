...
/** Tracing support for Eru effects.
  *
  * Provides structured tracing without external dependencies. Span completion is recorded on the
  * trace context; no observer events are emitted.
  *
  * Features:
  *   - Trace context with span hierarchy
  *   - Span tags and events
  *   - Trace baggage that flows through the effect
  */
object EruTrace {
  ...
  object SpanId {
    ...
    /** Process-unique ID generation for SpanId.
      *
  ...
  object TraceId {
    ...
    /** Process-unique ID generation for TraceId.
      *
      * Layout: [0][15-bit processId][48-bit timestamp/counter]
      *   - Bit 63: Always 0 (ensures positive Long)
      *   - Bits 62-48: Process identifier (15 bits = 32K unique processes)
  ...
  /** Structured information about a span in a trace.
    *
    * A span is a unit of work within a trace, with timing information and contextual metadata for
    * debugging and performance analysis. Each span captures the execution context of a specific
    * operation, including its relationship to parent spans.
    *
    * Key features:
  ...
  final class Span(
    ...
    /** Adds an event to this span's timeline.
      *
      * Events are occurrences recorded during span execution; they form a timeline for debugging
  ...
  /** An event that occurred during span execution.
    *
    * Span events are milestones or checkpoints recorded during the execution of a traced operation.
    * They form a timeline for debugging and performance analysis.
    *
    * Events are useful for:
    *   - Marking milestones in complex operations
  ...
  /** Trace context that flows through effect execution.
    *
    * TraceContext carries the current span and the baggage that flows through the trace. The
    * context is held in a thread-local and does not propagate across process or network boundaries.
    *
