...
/** Observer API for receiving structured execution events during program runs.
  ...
  * `ProgramEnd`, and `Step`. `FiberStarted` and `FiberCompleted` are emitted when fiber operations
  * are used.
  */
object EruObserver {
  ...
  /** An observer that discards all events.
    */
  def noop: EruObserver = (_: EruEvent) => ()

  /** An observer that prints events to standard output.
    */
  def console: EruObserver = (event: EruEvent) => {
  ...
  object ScopeId {
    private val processUniqueStart = {
      ...
      (processId << 48) | (nanoTime & 0xffffL) | 0x3000L
    }
  ...
  enum Outcome {
    ...
    /** Successful program completion.
      *
      * The success value is not part of the outcome.
      */
    case Success

    /** Program failure with a typed, domain-specific error.
      *
      * The program failed through Eru's typed error channel. The error is part of the program's
      * domain logic, not a defect.
      *
      * @param error
    ...
    /** Program failure due to an unexpected throwable (defect).
      *
      * A defect is a throwable that escaped the typed error channel.
      *
      * @param throwable
  ...
  /** Structured events emitted by Eru's observer-aware interpreter.
    ...
    *   - '''Tracing Events:''' Provide tracing information for performance analysis
    *
    * Each event carries its scope and fiber identifiers for correlation across events.
    *
    * @example
    *   {{{
  ...
  enum EruEvent {
    ...
    /** Signals the start of a new program execution.
      *
      * This event is emitted when a new Eru program begins execution via `unsafeRunSyncWith`. It
      * marks the beginning of an execution scope and provides the ScopeId for correlating
      * subsequent events.
      *
      * @param scopeId
    ...
    /** Signals an intermediate step or checkpoint during program execution.
      *
      * This event is emitted when an effect wrapped with `debug(label)` executes. It is intended
      * for low-volume, human-readable debugging traces, not for high-frequency metrics.
      *
      * @param scopeId
      *   the unique identifier for this program execution scope
      * @param label
    ...
    /** Signals the interruption of a fiber with the interruption cause.
      *
      * This event is defined for cancellation observability, but the current interpreter does not
      * emit it: interrupted fibers complete with `FiberCompleted` carrying an `Exit.Interrupt`.
      *
      * @param fiberId
    ...
    /** Signals the establishment of a parent-child fiber relationship.
      *
      * This event is defined for structured-concurrency tracking, but the current interpreter does
      * not emit it.
      *
      * @param parentId
    ...
    /** Signals the beginning of structured cleanup for a fiber's children.
      *
      * Emitted by `EruRuntime.shutdownRootFibers` (the observable shutdown path). The automatic
      * per-scope unwind of structured concurrency is not observer-visible.
      *
      * @param fiberId
    ...
    /** Signals the interruption request sent to a child fiber during structured cleanup.
      *
      * Emitted by `EruRuntime.shutdownRootFibers` (the observable shutdown path), with the
      * interruption cause carried verbatim. Root fibers' parent is the runtime itself, named by
      * [[FiberId.Root]].
      *
      * @param parentId
    ...
    /** Signals the completion of a tracing span for performance analysis.
      *
      * This event is defined for tracing observability, but the current interpreter does not emit
      * it: spans completed by `traced` are recorded on the trace context, not sent to observers.
      *
      * @param span
  ...
  /** Observer interface for receiving structured execution events.
    ...
    * events. Implementations receive events as they occur and can perform side-effecting operations
    * such as logging, metrics collection, tracing, and alerting.
    *
    * The method is called synchronously during execution; implementations should be fast and
    * non-blocking.
    *
  ...
  trait EruObserver {
    ...
    /** Handles a structured execution event.
      *
      * This method is called for each event during Eru program execution. Implementations should
      * handle events efficiently and never throw exceptions.
      *
