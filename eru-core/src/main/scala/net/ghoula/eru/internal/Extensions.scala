...
object Extensions {
  ...
  extension [E, A](eru: Eru[E, A]) {
    ...
    /** Wraps this effect with a trace span for observability.
      *
      * This method creates a new span for this effect's execution, providing timing and context
      * information for debugging and performance analysis. The completed span is recorded on the
      * trace context; no observer event is emitted.
      *
      * The ambient trace context is restored when the effect completes as an execution-ordered
      * step, not a finalizer: nested scopes close in evaluation order (innermost last), so the
      * ambient context is always restored to what preceded the innermost completed scope.
      *
      * @param operation
    ...
    def traced(operation: String, tags: Map[String, String] = Map.empty): Eru[E | Throwable, A] = {
      ...
      Eru.effectTotal {
        ...
        val context = prior.getOrElse(startTrace("root-trace"))
        ...
        (prior, taggedSpan)
      }.flatMap { case (prior, span) =>
        val body: Eru[E, A] = eru.attempt.map { result =>
          result match {
        ...
        body.attempt.flatMap { r =>
          Eru.effectTotal { setCurrentContext(prior); () }.flatMap(_ =>
            r match {
              case Result.Success(value) => Eru.succeed(value)
              case Result.Failure(error) => Eru.fail(error)
            }
    ...
    /** Adds baggage (trace-wide context) to the current trace.
      ...
      *
      * The baggage is scoped to this effect: the ambient trace context is restored exactly when the
      * effect completes.
      *
      * @param key
      *   baggage key
    ...
    def withTraceBaggage(key: String, value: String): Eru[E, A] = {
      ...
      Eru.effectTotal {
        val prior = getCurrentContext
        prior match {
          case Some(context) => setCurrentContext(Some(context.withBaggage(key, value)))
          case None => setCurrentContext(Some(startTrace("implicit-trace").withBaggage(key, value)))
        }
        prior
      }.flatMap { prior =>
        eru.attempt.flatMap { r =>
          Eru.effectTotal { setCurrentContext(prior); () }.flatMap(_ =>
            r match {
              case Result.Success(value) => Eru.succeed(value)
              case Result.Failure(error) => Eru.fail(error)
