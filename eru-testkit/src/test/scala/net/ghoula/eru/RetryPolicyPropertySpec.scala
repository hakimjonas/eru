...
final class RetryPolicyPropertySpec extends munit.ScalaCheckSuite {
  ...
  /** Validates that Policy.NoDelay bounds retry attempts and succeeds appropriately.
    *
    * Property test that verifies NoDelay policy limits attempts to the specified maximum and
    * succeeds when the success condition is met within the retry limit.
    */
  property("Policy.NoDelay(n) bounds attempts and succeeds if successIndex <= n + 1") {
    forAll(nonNegativeSmall, positiveSmall) { (maxRetries, successIndexRaw) =>
      ...
      val retried = eff.retry(EruRuntime.Policy.NoDelay(maxRetries))
