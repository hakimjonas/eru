...
/** Test utility providing isolation between test executions.
  *
  * Each test runs with a fresh backend instance, preventing state leakage between tests. The
  * backend's cleanup() method is always called after test completion, even if the test fails or
  * throws an exception.
  */
object IsolatedTestRunner {

  /** Isolated runtime backed by a fresh backend per test.
    *
    * This class wraps a fresh backend instance and provides a subset of the EruRuntime API: fork,
    * race, timeout, sleep, parSequence, zipPar, raceAll, suspend, and cleanup. Use this in tests
    * instead of the global EruRuntime to avoid state interference.
    */
  class IsolatedRuntime(private val backend: ConcurrencyBackend, val testClock: TestClock) {
    ...
    /** Parallel sequence over the isolated backend.
      *
      * Interruptions are reported first; otherwise the first failing fiber's outcome (typed error
  ...
  /** Creates an isolated runtime for test execution.
    ...
    * tests. Remember to call cleanup() after your test completes.
    */
  ...
  /** Executes an Eru computation with test isolation.
    *
    * Creates a fresh backend that is cleaned up after execution completes. The computation itself
    * runs on the global interpreter; the fresh backend only isolates the runtime's fiber tracking.
    *
    * @param computation
  ...
  /** Executes an Eru computation with test isolation and observer support.
    *
    * Creates a fresh backend that is cleaned up after execution completes. The computation itself
    * runs on the global interpreter; the fresh backend only isolates the runtime's fiber tracking.
    *
  ...
  /** Executes an Eru computation that may fail with test isolation.
    *
    * Creates a fresh backend that is cleaned up after execution completes. The computation itself
    * runs on the global interpreter; the fresh backend only isolates the runtime's fiber tracking.
    * Returns the Exit result instead of throwing exceptions.
    *
    * @param computation
    *   the Eru computation to execute
    * @return
  ...
  /** Executes an Eru computation that may fail with test isolation and observer.
    *
    * Creates a fresh backend that is cleaned up after execution completes. The computation itself
    * runs on the global interpreter; the fresh backend only isolates the runtime's fiber tracking.
    * Returns the Exit result instead of throwing exceptions.
